...
\begin{figure*}[t!]
\begin{center}
\includegraphics[width=0.85\textwidth]{images/fig1_composite.pdf}
...
This is the form of output that latent graph learners structurally cannot produce: a named, falsifiable claim --- \emph{``spectral Granger causality in the 0.25--0.5\,Hz band is the dominant coupling mode''} --- derived end-to-end from data. The natural next step is clinical EEG, where the question shifts from ``can we recover known causal structure?'' to ``what form of inter-channel dependence predicts seizure onset?'' We are awaiting access approval for the TUH EEG Corpus to test exactly this.
